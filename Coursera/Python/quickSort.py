...
def partition_head(array, head, tail):
    """
    Partitions an 'array' with its first element as 'pivot'.
    ...
    swap is between pivot and the last element of left half.
    """
    global count
    pivot = array[head]
    pointer = head + 1
...
def main():
    ...
    input_file = open('quickSortAssignement1.txt').read().strip().split()
